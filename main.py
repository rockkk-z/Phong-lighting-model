...
# --- 阴影检测函数 (已修复Taichi语法错误) ---
@ti.func
def is_in_shadow(hit_point, light_pos):
    """
    硬阴影检测：从交点发射阴影射线到光源
    返回True：在阴影中 | 返回False：不在阴影中
    """
    in_shadow = False  # 用变量标记，统一返回
    shadow_origin = hit_point
    # 阴影射线方向
    shadow_dir = normalize(light_pos - shadow_origin)
    # 射线最大长度（到光源的距离）
    max_t = (light_pos - shadow_origin).norm()
    
    # 偏移原点，避免自身相交（解决阴影 acne）
    shadow_origin += shadow_dir * 1e-3
    
    # 检测是否与球体相交
    t_sph, _ = intersect_sphere(shadow_origin, shadow_dir, ti.Vector([-1.2, -0.2, 0.0]), 1.2)
    if 0 < t_sph < max_t:
        in_shadow = True
    
    # 检测是否与圆锥相交
    t_cone, _ = intersect_cone(shadow_origin, shadow_dir, ti.Vector([1.2, 1.2, 0.0]), -1.4, 1.2)
    if 0 < t_cone < max_t:
        in_shadow = True
